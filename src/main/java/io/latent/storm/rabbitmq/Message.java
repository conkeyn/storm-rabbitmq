...
import org.apache.storm.zookeeper.proto.ReplyHeader;
...
public class Message
{
	...
	{
		return (body != null && ((exchangeName != null && exchangeName.length() > 0)||(routingKey != null && routingKey.length() > 0))) ? new MessageForSending(body, headers, exchangeName, routingKey, contentType, contentEncoding, persistent) : NONE;
	}

	public static Message forSending(byte[] body, Map<String, Object> headers, String exchangeName, String routingKey, String contentType, String contentEncoding, boolean persistent, String replyTo)
	{
		Message mfs = NONE;
		if (null != body && null != replyTo && !replyTo.isEmpty())
		{
			mfs = new MessageForSending(body, headers, replyTo, contentType, contentEncoding, persistent);
		}
		else if (body != null && exchangeName != null && exchangeName.length() > 0)
		{
			mfs = new MessageForSending(body, headers, exchangeName, routingKey, contentType, contentEncoding, persistent);
		}
		return mfs;
	}
	...
	public static class MessageForSending extends Message
	{
									...
									boolean persistent)
		{
			...
			Object replyToObj =headers.get("replyTo");
			if(replyToObj!=null){
				this.replyTo = replyToObj.toString();
			}else{
				this.replyTo =null ;
			}
		...
		public MessageForSending(byte[] body, Map<String, Object> headers, String replyTo, String contentType, String contentEncoding, boolean persistent)
		{
			super(body);
			this.headers = (headers != null) ? headers : new HashMap<String, Object>();
			Object replyToObj =headers.get("replyTo");
